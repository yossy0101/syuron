...
from conv import fc_ReLU_Drop
from oct_pool import Oct_MaxPool2d
...
class VGG(nn.Module):
    ...
    def _make_layer(self, out_channels, blocks, input=False, output=False):
        ...
        # if input == True:
        #     layers.append(init_Oct_BN_ReLU(self.in_channels, out_channels, kernel_size=3, stride=1, padding=1, in_alpha=in_alpha, out_alpha=self.alpha))
        # else:    
        ...
        for i in range(1, blocks):
            ...
            # if input==True:
            #     layers.append(second_Oct_BN_ReLU(self.in_channels, out_channels, kernel_size=3, stride=1, padding=1, in_alpha=self.alpha, out_alpha=out_alpha))
            # else:
            #     layers.append(Oct_BN_ReLU(self.in_channels, out_channels, kernel_size=3, stride=1, padding=1, in_alpha=self.alpha, out_alpha=out_alpha))
