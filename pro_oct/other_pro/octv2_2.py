...
class OctConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, in_alpha=0.0, out_alpha=0.0, bias=True):
        ...
        #HtoL
        if self.kernel == 1 and not (self.ch_out_lf == 0 or self.ch_out_hf == 0):
            self.HtoL = nn.Conv2d(self.ch_in_hf, self.ch_out_lf, kernel_size, stride, padding, bias=bias)
        elif not (self.ch_out_lf == 0 or self.ch_out_hf == 0):
            self.HtoL = nn.Conv2d(self.ch_out_hf, self.ch_out_lf, kernel_size=1, padding=0, bias=bias)
        #LtoH
        if (self.ch_out_lf == 0 or self.kernel == 1) and not (self.ch_out_hf == 0 or self.ch_in_lf == 0):
    ...
    def forward(self, x):
        ...
        if self.HtoH is not None:
            oHtoH = self.HtoH(hf)
        if self.kernel == 1 and self.HtoL is not None:
            oHtoL = self.HtoL(self.downsample(hf))
        elif self.HtoL is not None:
            oHtoL = self.HtoL(self.downsample(oHtoH))
        if self.LtoL is not None:
            oLtoL = self.LtoL(lf)
        if (self.ch_out_lf == 0 or self.kernel == 1) and self.LtoH is not None:
