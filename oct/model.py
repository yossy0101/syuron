...
def make_model(model='resnet18', num_classes=100, alpha=0.0):
    if model=='vgg16':
        return model, vgg16(num_classes, alpha)
    elif model=='vgg19':
        return model, vgg19(num_classes, alpha)
    elif model=='resnet18':
        return model, resnet18(num_classes, alpha)
    elif model=='resnet34':
        return model, resnet34(num_classes, alpha)
    elif model=='resnet50':
        return model, resnet50(num_classes, alpha)
    elif model=='resnet101':
        return model, resnet101(num_classes, alpha)
    else:
